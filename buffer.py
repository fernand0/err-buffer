...
class Buffer(BotPlugin):
                    ...
                    for val in valueE:
                        ...
                        if service == "direct":
                            ...
                            if 'posts' in config[section]:
                                fromSrv = (fromSrv[0], fromSrv[1], fromSrv[2],
            ...
            for service in services["regular"]:
                ...
                if service in config.options(section):
                    ...
                    for method in methods:
                        ...
                        if toAppend:
                            if hasSpecial: 
                                fromSrvSp = ('cache', 'set', (fromSrv[2], 
                                    (toAppend[2], toAppend[3])), 'posts')
        ...
        if pos >= 0:
            for element in myList:
                ...
                if hasattr(self.clients[(element, profile, name)], 
                    ...
                    if self.clients[(element, profile, 
                                     name)].getPostsType() == 'drafts': 
                        postsTmp = self.clients[(element, profile, 
                                                 name)].getDrafts() 
                    else: 
                        postsTmp = self.clients[(element, profile, 
                                                 name)].getPosts()
                ...
                if postsTmp:
                    for (i, post) in enumerate(postsTmp):
                        if (hasattr(self.clients[(element, profile, name)], 
                            'getPostLine')):
                            title = self.clients[(element, profile, 
                                                  name)].getPostLine(post)
                            link = ''
                        else: 
                            title = self.clients[(element, profile, 
                                                  name)].getPostTitle(post)
                            link = self.clients[(element, profile, 
                                                 name)].getPostLink(post)
    ...
    def publish(self, mess, args):
        ...
        dst = (available[args[0].lower()]['name'], 'set', 
        ...
        for action in actions:
            ...
            if action[0] == 'cache':
                ...
                #yield(apiDst)
                res = apiDst.addPosts( [apiSrc.obtainPostData(j), ])
            else:
    ...
    def prepareReply(self, updates, types):
        ...
        for socialNetwork in updates.keys():
            ...
            if isinstance(data['data'][pos][1], str):
                ...
                socNick = f"{data['data'][pos][1][1][0].capitalize()} "\
                          f"{data['data'][pos][1][1][1]}"
            logging.info(f"Nick ... {social}")
            logging.info(f"Nick ... {socNick}")
            socialNetworktxt = f"{social.capitalize()} "\
                               f"({name.capitalize()} - {socNick})"
            if theUpdates: 
